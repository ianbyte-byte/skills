#!/usr/bin/env python3
"""Validate plugin.json and mcp.json against the Agent Plugins v1.0.0 JSON Schemas.

Positive: the package's own manifests must validate.
...
Run from the repo root:
    ...
    python3 agent-plugins-spec/scripts/validate.py
"""
...
def main() -> int:
    ...
    plugin_data = json.loads((ROOT / "plugin.json").read_text())
    mcp_data = json.loads((ROOT / "mcp.json").read_text())
    ...
    bad = copy.deepcopy(mcp_data)
    bad["mcpServers"]["registry-example"].pop("url")
    ok &= _expect_rejected("mcp.json streamable-http missing url (§7.2.1)", mcp_schema, bad)
